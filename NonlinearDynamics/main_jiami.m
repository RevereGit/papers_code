%% 基于混沌系统与DNA编码的彩色数字图像加密
clear;
clc;
%path='E:\Database\ALL_REF\LivingRoom\';
%img_list=dir(path);
%for k=10:99
% I=imread(['E:\Database\ALL_REF\LivingRoom\Frame_0',num2str(k),'.png']);         %读取图像信息
I=imread('F:\Users\Revere\Desktop\Encryption\paper-LFI-encryption-DNA\原始、加密、解密图片\living.png'); 
I1=I(:,:,1);        %R
I2=I(:,:,2);        %G
I3=I(:,:,3);        %B
...
%%figure;imhist(I1);title('The R channel histogram of the plain image');
%%figure;imhist(I2);title('The G channel histogram of the plain image');
%%figure;imhist(I3);title('The B channel histogram of the plain image');
...
%水平 B通道随机点像素灰度值  , 与该点相邻水平方向像素灰度值,  原始图像B通道水平相关性曲线
%figure;scatter(XX_R_SP,YY_R_SP,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent horizontal direction');title('The R channel horizontal correlation curve of the plain image');
%figure;scatter(XX_G_SP,YY_G_SP,18,'filled');xlabel('The random pixel gray value of the G channel');ylabel('The pixel gray value in the adjacent horizontal direction');title('The G channel horizontal correlation curve of the plain image');
%figure;scatter(XX_B_SP,YY_B_SP,18,'filled');xlabel('The random pixel gray value of the B channel');ylabel('The pixel gray value in the adjacent horizontal direction');title('The B channel horizontal correlation curve of the plain image');
%垂直  R通道随机点像素灰度值, 与该点相邻垂直方向像素灰度值, 原始图像R通道垂直相关性曲线
% figure;scatter(XX_R_CZ,YY_R_CZ,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent vertical direction');title('The R channel vertical correlation curve of the plain image');
% figure;scatter(XX_G_CZ,YY_G_CZ,18,'filled');xlabel('The random pixel gray value of the G channel');ylabel('The pixel gray value in the adjacent vertical direction');title('The G channel vertical correlation curve of the plain image');
% figure;scatter(XX_B_CZ,YY_B_CZ,18,'filled');xlabel('The random pixel gray value of the B channel');ylabel('The pixel gray value in the adjacent vertical direction');title('The B channel vertical correlation curve of the plain image');
%对角线  R通道随机点像素灰度值  , 与该点相邻对角线方向像素灰度值, 原始图像R通道对角线相关性曲线
% figure;scatter(XX_R_DJX,YY_R_DJX,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent diagonal direction');title('The R channel diagonal correlation curve of the plain image');
% figure;scatter(XX_G_DJX,YY_G_DJX,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent diagonal direction');title('The G channel diagonal correlation curve of the plain image');
% figure;scatter(XX_B_DJX,YY_B_DJX,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent diagonal direction');title('The B channel diagonal correlation curve of the plain image');
%R通道
...
x0=sum(I1(:))/(255*SUM);     %计算得出Logistic初值x0    I1(:)按照每列纵向排列，将行与列值所有元素排为一列，x0=1/255=0.0039
x0=floor(x0*10^4)/10^4;     %保留4位小数   floor函数取整
p=zeros(1,SUM+1000);        %预分配内存   返回一个1*（SUM+1000）大小的零矩阵
p(1)=x0;
...
%求四个初值X0,Y0,Z0,H0
%(4)
r=(M/t)*(N/t);      %r为分块个数，  M=720，N=960， M/t=720/4=180，N/t=240，  r=180*240=43200  个块
...
figure;imhist(Q_R);title('The R channel histogram of the encrypted image');
axis([0 255 0 5000]);
figure;imhist(Q_G);title('The G channel histogram of the encrypted image');
axis([0 255 0 5000]);
figure;imhist(Q_B);title('The B channel histogram of the encrypted image');
axis([0 255 0 5000]);
Q_jiami(:,:,1)=Q_R;
Q_jiami(:,:,2)=Q_G;
Q_jiami(:,:,3)=Q_B;
...
%%figure;imshow(Q_jiami); title('加密后图片');
   ...
   %计算npcr和uaci值
% Q_jiamiR=Q_jiami(:,:,1);
npcr_r=mesure_npcr(I(:,:,1),Q_jiami(:,:,1));   
npcr_g=mesure_npcr(I(:,:,2),Q_jiami(:,:,2)); 
npcr_b=mesure_npcr(I(:,:,3),Q_jiami(:,:,3)); 
uaci_r=mesure_uaci(I(:,:,1),Q_jiami(:,:,1));
uaci_g=mesure_uaci(I(:,:,2),Q_jiami(:,:,2));
uaci_b=mesure_uaci(I(:,:,3),Q_jiami(:,:,3));


% f=getframe(gcf);
% imwrite(f.cdata,['..\原始、加密、解密图片\pic\',int2str(i),'.png']);   
%% 加密后信息熵
%R通道
T2_R=imhist(Q_R);
S2_R=sum(T2_R);
...
%水平  G通道随机点像素灰度值,与该点相邻水平方向像素灰度值, 加密图像G通道水平相关性曲线
%%figure;scatter(XX_R_SP,YY_R_SP,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent horizontal direction');title('The R channel horizontal correlation curve of the encrypted image');
%%figure;scatter(XX_G_SP,YY_G_SP,18,'filled');xlabel('The random pixel gray value of the G channel');ylabel('The pixel gray value in the adjacent horizontal direction');title('The G channel horizontal correlation curve of the encrypted image');
%%figure;scatter(XX_B_SP,YY_B_SP,18,'filled');xlabel('The random pixel gray value of the B channel');ylabel('The pixel gray value in the adjacent horizontal direction');title('The B channel horizontal correlation curve of the encrypted image');
%垂直
%%figure;scatter(XX_R_CZ,YY_R_CZ,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent vertical direction');title('The R channel vertical correlation curve of the encrypted image');
%%figure;scatter(XX_G_CZ,YY_G_CZ,18,'filled');xlabel('The random pixel gray value of the G channel');ylabel('The pixel gray value in the adjacent vertical direction');title('The G channel vertical correlation curve of the encrypted image');
%%figure;scatter(XX_B_CZ,YY_B_CZ,18,'filled');xlabel('The random pixel gray value of the B channel');ylabel('The pixel gray value in the adjacent vertical direction');title('The B channel vertical correlation curve of the encrypted image');
%对角线
%%figure;scatter(XX_R_DJX,YY_R_DJX,18,'filled');xlabel('The random pixel gray value of the R channel');ylabel('The pixel gray value in the adjacent diagonal direction');title('The R channel diagonal correlation curve of the encrypted image');
%%figure;scatter(XX_G_DJX,YY_G_DJX,18,'filled');xlabel('The random pixel gray value of the G channel');ylabel('The pixel gray value in the adjacent diagonal direction');title('The G channel diagonal correlation curve of the encrypted image');
%%figure;scatter(XX_B_DJX,YY_B_DJX,18,'filled');xlabel('The random pixel gray value of the B channel');ylabel('The pixel gray value in the adjacent diagonal direction');title('The B channel diagonal correlation curve of the encrypted image');
%R通道
Q_R=double(Q_R);
...
%% 输出数据信息
%imwrite(Q_jiami,['F:\Users\Revere\Desktop\Encryption\paper-LFI-encryption-DNA\原始、加密、解密图片\encrypt\',int2str(k),'.png']);
%end
disp('加密成功');
disp('密钥：');
